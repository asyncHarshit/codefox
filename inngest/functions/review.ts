...
import { getPullRequestDiff, postReviewComment } from "@/lib/github";
...
export const generateReview = inngest.createFunction(
    ...
    async({event , step})=>{
        ...
        const {diff , title , description , token} = await step.run("fetch-pr-data",async()=>{
            ...
            const data = await getPullRequestDiff(account.accessToken , owner , repoName , prNumber)
        ...
        const review = await step.run("generate-ai-review" , async()=>{
            const prompt = `const prompt = You are an expert code reviewer. Analyze the following pull request and provide a detailed, constructive code review.
                PR Title: ${title}
